...
const BigResult = ({ result }: ResultProps) => {
  const { l } = usePage();
  return (
    <div className=" sm:flex-row flex flex-col mt-4 mb-2 sm:gap-8 sm:px-6">
...
const SmallResult = ({ result }: ResultProps) => {
  ...
  return (
    <div className="grid grid-cols-[max-content_1fr] gap-x-2 gap-y-6  relative mb-2">
      <div className="text-syrs-label text-sm text-nowrap pt-6">{l("result.skinAgingLevel")}</div>
      <div></div>
      {result.data.skinAgingImprovement ? (
        <>
          <div className="col-span-1 text-syrs-label text-center text-4xl font-semibold">
            {result.data.skinAgingImprovement}
          </div>
          <div className="col-span-1 text-syrs-font text-opacity-70 text-sm">{result.data.skinAgingDesc}</div>
        </>
      ) : (
        <div className="col-span-2 text-syrs-font text-opacity-70 text-sm">{result.data.skinAgingDesc}</div>
      )}

      <div className="text-syrs-label text-sm text-nowrap pt-6 ">
        <hr className="w-full absolute -mt-6 border-t-[0.5px]" />
      ...
      {result.data.sensitivityImprovement ? (
        <>
          ...
          <div className="col-span-1 text-syrs-font text-opacity-70 text-sm">{result.data.sensitivityDesc}</div>
        </>
      ) : (
      ...
      {result.data.oilWaterBalanceImprovement ? (
        <>
          ...
          <div className="col-span-1 text-syrs-font text-opacity-70 text-sm">{result.data.oilWaterBalanceDesc}</div>
        </>
      ) : (
