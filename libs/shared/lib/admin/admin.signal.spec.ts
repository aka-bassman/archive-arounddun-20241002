...
export interface AdminAgent<Fetch = typeof fetch> {
  ...
  adminInput: db.AdminInput & { password: string };
}
...
export const getAdminAgentWithInitialize = async <Fetch = typeof fetch>(): Promise<AdminAgent<Fetch>> => {
  ...
  // 3. Admin 로그인
  const accessToken = await fetch.signinAdmin(adminInput.accountId, adminInput.password);
  ...
  return { admin, fetch: adminFetch as Fetch, accessToken, adminInput };
};
