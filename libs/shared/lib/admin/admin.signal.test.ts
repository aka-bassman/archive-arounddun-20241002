...
import { sample } from "@core/common";
...
describe("Admin Signal", () => {
  describe("Admin Service", () => {
    ...
    it("can signout admin", async () => {
      // 1. Admin 로그인
      const decodedAdminJwt = decodeJwt<Account | null>(adminAgent.fetch.client.getJwt() ?? "");
      expect(decodedAdminJwt?.me).toBeTruthy();

      // 2. Admin 로그아웃
      const { jwt: updatedAdminJwt } = await adminAgent.fetch.signoutAdmin();
      const decodedUpdatedAdminJwt = decodeJwt<Account | null>(updatedAdminJwt);
    ...
    it("can remove admin", async () => {
      ...
      // 2. Admin 로그인 불가
      await expect(
        adminAgent.fetch.signinAdmin(adminAgent.adminInput.accountId, adminAgent.adminInput.password)
      ).rejects.toThrow();
    });
  });
});
