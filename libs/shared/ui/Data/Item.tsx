...
import { type BaseObject, Dayjs } from "@core/base";
import { DataAction, DataColumn, clsx } from "@core/client";
import { Dropdown, ObjectId, Popconfirm, RecentTime } from "@util/ui";
import { capitalize, prettyPrint } from "@core/common";
...
export default function Item<T extends string, Full extends { id: string }, Light extends { id: string }>({
  ...
  const extraCols = columns
    ...
    .map((column, idx) => {
      const key = (typeof column === "string" ? column : column.key) as string;
      const title = typeof column !== "string" && column.title ? column.title : l.field(sliceName as any, key);
      const render = convToAntdColumn(column).render ?? ((v: any, m: any, i: number) => prettyPrint(v));
