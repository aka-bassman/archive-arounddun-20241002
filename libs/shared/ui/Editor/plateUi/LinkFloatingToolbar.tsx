...
export const LinkFloatingToolbar = ({ state }: LinkFloatingToolbarProps) => {
  ...
  const popupClassName = "w-auto p-1 rounded-md border border-black/20 bg-base-100 shadow-md outline-none";

  return (
    <>
      <div ref={insertRef} className={popupClassName} {...insertProps}>
        {input}
      </div>
      <div ref={editRef} className={popupClassName} {...editProps}>
        {editContent}
      </div>
    </>
  );
