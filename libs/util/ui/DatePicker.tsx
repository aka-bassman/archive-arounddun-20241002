...
export const DatePicker = ({
  ...
  const handleDateChange = (date?: Date | null) => {
    if (!date) {
      msg.warning("util.selectDateError");
      return;
    }
    onChange(dayjs(date));
  };
  ...
  return (
    <ReactDatePicker
      className={clsx("input input-bordered text-center", className)}
...
const RangePicker = ({
  ...
  const pickerClassName = "m-0 input input-boarded focus:outline-none z-50 p-3 text-center h-full w-full ";
  return (
    <div className={clsx("input input-bordered flex h-full w-fit  items-center gap-2 p-0", className)}>
      <ReactDatePicker
...
const TimePicker = ({
  ...
  return (
    <ReactDatePicker
      ...
      className={clsx("input input-bordered inline-block w-auto", className)}
      selected={value ? value.toDate() : new Date()}
      onChange={handleDateChange}
      showTimeSelect
      showTimeSelectOnly
