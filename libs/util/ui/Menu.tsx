...
export const Menu = ({
  ...
  const subMenuClassName =
    mode === "horizontal"
      ? `fixed menu-title bottom-0 translate-y-[98%] border-0 rounded-sm shadow-lg bg-base-100 hover:bg-base-100 flex flex-col`
            ...
            return (
              <li
                ...
                className={clsx("relative m-0 duration-200 hover:opacity-70", overflowClassName, liClassName, {
                  ...
                  "border-b-2 border-base-100": activeStyle === "bordered" && checkIsActive(item.key),
                })}
                onClick={() => {
              ...
              >
                <div className="flex h-full justify-between rounded-none">
                  ...
                  {item.children && mode === "inline" && (
                    <AiFillCaretDown
                      className={clsx(
                        "duration-400 text-xs transition-transform",
...
const OverflowMenu = ({ overflowItems, onClick }: OverflowMenuProps) => {
  ...
  return (
    <li className="relative h-full" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      ...
      {isOpen && (
        <div className="bg-base-100 hover:bg-base-100 fixed  -bottom-0 flex translate-y-[98%] flex-col rounded-sm border-0 p-2 shadow-lg">
          {overflowItems.map((item) => (
            <div
              key={item.key}
              onClick={() => onClick?.(item)}
              className="btn btn-ghost text-primary-focus relative whitespace-nowrap font-normal"
              onMouseEnter={() => {
            ...
            >
              {item.label}
              {item.children && expandedKey === item.key && (
                <div className="bg-base-100 absolute left-0 top-0 -translate-x-full p-4 drop-shadow">
                  {item.children.map((child) => (
                    <div
                      onClick={(e) => {
                        e.stopPropagation();
