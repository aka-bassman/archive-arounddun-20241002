...
export default function executor(
  ...
  const isCameraPluginInstalledIos = iosPlist.includes(`<key>NSCameraUsageDescription</key>`);
  if (isCameraPluginInstalled && isCameraPluginInstalledIos) {
    logger.error(`Camera plugin is already installed`);
    return { success: false };
  }
  //* 3. Sync and run
  runInWorkspace(
    `npx trapeze run ./pkgs/codebase/executors/add-camera/config.yaml --android-project ${projectRoot}/android --ios-project ${projectRoot}/ios/App`
  );
