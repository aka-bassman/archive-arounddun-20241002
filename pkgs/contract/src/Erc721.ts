...
import type { ContractSettings, Erc721EventHandler, Erc721Info } from ".";
import type { ERC721AToken } from "../typechain-types";

export class Erc721 {
  constructor(
    readonly address: string,
    readonly contract: ERC721AToken,
  ...
  async info(): Promise<Erc721Info> {
    ...
    ).map((ret) => [ret[0], ret[1]]) as unknown as [[string, number], [string], [number]];
    ...
    const tokenMap: {
      ...
      balanceMap.map(async (b) => {
        ...
        try {
          ...
          const tokens = (tokenIds as unknown as number[]).map((tokenId) => ({
            address: owner,
            contract: b.contract,
        ...
        try {
          const calls = balanceMap.reduce(
            (acc, map) => [
              ...acc,
              ...new Array(map.value).fill(0).map((_, idx) => ({
          ...
          const tokens = (
            ...
            tokenId: parseInt((ret[0] as unknown as number).toString()),
            value: 1,
            bn: ret[1],
          }));
          const uris = await this.tokenURIs(
  ...
  async getSaleInfo(key: number) {
    ...
    return {
      ...
      price: parseFloat(formatEther(ret[1].toString())),
  ...
  async balances(owners: string[]) {
    ...
    ).map((ret) => parseInt((ret[0] as unknown as string).toString()));
    return balances;
  }
  async tokenURIs(tokenIds: number[], address = this.address) {
    ...
    ).map((ret, idx) => ({ tokenId: tokenIds[idx], uri: ret[0] as unknown as string }));
    return uris;
  }
  ...
  async transfer(from: string, to: string, tokenId: number) {
    await this.settings.market.transferErc721(this.address, from, to, tokenId, {
      gasLimit: 300000,
    });
    return true;
  }
  ...
  async #ownersOf(tokenIds: number[]) {
    ...
    ).map((ret) => ({ address: (ret[0] as unknown as string).toLowerCase(), bn: ret[1], value: 1 }));
    return owners;
  }
  async #tokenIdsOfOwner(owner: string): Promise<number[]> {
    ...
    try {
      ...
      const tokenIds = (
        await this.settings.multicall.view({
          calls: new Array(balance).fill(0).map((_, idx) => ({
            address: this.address,
            fn: "tokenOfOwnerByIndex",
            args: [owner, idx],
          })),
  ...
  async #tokenIdsAll(): Promise<number[]> {
    try {
      ...
      const tokenIds = (
        await this.settings.multicall.view({
          calls: new Array(supply).fill(0).map((_, idx) => ({
            address: this.address,
            fn: "tokenByIndex",
            args: [idx],
          })),
