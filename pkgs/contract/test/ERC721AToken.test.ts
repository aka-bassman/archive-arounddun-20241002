...
//       await expect(
//         contract.connect(wlUser).mint(1, saleInfos[1].perTx + 1, proof, {
//           value: etherToWei(saleInfos[1].perTx + 1),
//         })
//       ).to.be.reverted;
//       await expect(contract.connect(wlUser).mint(1, 1, proof, { value: etherToWei(1) })).not.to.be.reverted;
//       await expect(
//         contract.connect(wlUser).mint(1, saleInfos[1].perWallet, proof, {
//           value: etherToWei(saleInfos[1].perWallet),
//         })
//       ).to.be.reverted;
...
//       await contract.connect(root).setSaleInfoList(...saleInfosToArrays([saleInfos[2]]), 2);
//       await expect(
//         contract.connect(nonwlUser).mint(2, saleInfos[2].perTx + 1, [], {
//           value: etherToWei(saleInfos[2].perTx + 1),
//         })
//       ).to.be.reverted;
//       await expect(contract.connect(nonwlUser).mint(2, 1, [], { value: etherToWei(1) })).not.to.be.reverted;
//       await expect(
//         contract.connect(nonwlUser).mint(2, saleInfos[2].perWallet, [], {
//           value: etherToWei(saleInfos[2].perWallet),
//         })
//       ).to.be.reverted;
