...
const getLoaderMetaMap = (target: object): Map<string, LoaderMeta> => {
  const loaderMetaMap = Reflect.getOwnMetadata("loaders", target) ?? new Map<string, LoaderMeta>();
  return loaderMetaMap;
};
export const getLoaderMetas = (target: object): LoaderMeta[] => {
  const metas: LoaderMeta[] = [...getLoaderMetaMap(target).values()];
  return metas;
};
...
export const Loader = {
  ByField: (fieldName: string, defaultQuery: FilterQuery<unknown> = {}) => {
    return function (target: object, key: string) {
      const loaderMetaMap = getLoaderMetaMap(target);
